...
CLAIR_TMP_WRAPPER="${BIN_DIR}/quay-clair-tmp-clean.sh"
...
Environment variables (set before running):
  ...
  CLAIR_TMPFS_SIZE    Size cap for quay-clair's /tmp tmpfs (default: 4g)
...
# Size cap for quay-clair's tmpfs /tmp — bounds damage from leaked
# fetcher.*/rhel-vex.* updater dumps (see compose file comment for detail)
CLAIR_TMPFS_SIZE="${CLAIR_TMPFS_SIZE:-4g}"
...
services:
  ...
  quay-clair:
    ...
    tmpfs:
      # Clair 4.9.0's updater writes fetcher.* / rhel-vex.* vulnerability-DB
      # dumps (hundreds of MB to ~1.6G each) to /tmp and, on some update
      # failures, leaves them behind instead of cleaning up.  /tmp lives in
      # the container's writable layer by default, so those leaks eat host
      # disk under /var/lib/docker indefinitely.  Capping /tmp as tmpfs turns
      # a silent disk-fill into a bounded, self-limiting failure — the
      # container hits ENOSPC/OOMs and restarts instead.  quay-clair-tmp-clean.sh
      # purges stale dumps hourly so this cap is rarely hit in practice.
      - /tmp:size=${CLAIR_TMPFS_SIZE:-4g}
    depends_on:
      quay-db:
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Write Clair /tmp cleanup wrapper.
# Clair 4.9.0's updater leaves fetcher.*/rhel-vex.* vulnerability-DB dumps
# (hundreds of MB to ~1.6G each) behind in /tmp when an update run fails to
# complete cleanly — a known upstream leak, not something config can prevent.
# Runs hourly, independent of the daily GC and of Quay's own health, since it
# only needs quay-clair up.
__info "Writing ${CLAIR_TMP_WRAPPER}"
cat >"${CLAIR_TMP_WRAPPER}.tmp" <<'EOS'
#!/usr/bin/env sh
# shellcheck shell=sh
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
# @@Copyright        :  Copyright: (c) 2026 Jason Hempstead, Casjays Developments
# @@Created          :  Monday, June 01, 2026 00:46 EDT
# @@File             :  quay-clair-tmp-clean.sh
# @@Description      :  Purges stale Clair updater temp dumps (generated by install.sh)
# @@Changelog        :  New file
# @@TODO             :  None
# @@Other            :  Run via systemd timer or cron — never call directly
# @@Resource         :
...
if command -v docker >/dev/null 2>&1; then
  ...
  __ctc_log "No container engine found; skipping Clair tmp cleanup"
  exit 0
fi
if ! $_cexec quay-clair true >/dev/null 2>&1; then
  __ctc_log "quay-clair not running; skipping"
  exit 0
fi
# Only remove dumps older than 60 minutes — leaves in-progress updater runs alone
$_cexec quay-clair find /tmp -maxdepth 1 \
  \( -name 'fetcher.*' -o -name 'rhel-vex.*' \) -mmin +60 -delete 2>/dev/null || true
__ctc_log "Clair tmp cleanup completed"
exit 0
# ex: ts=2 sw=2 et filetype=sh
EOS
\mv "${CLAIR_TMP_WRAPPER}.tmp" "$CLAIR_TMP_WRAPPER"
\chmod 700 "$CLAIR_TMP_WRAPPER"
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
EOF
  cat >/etc/systemd/system/quay-clair-tmp-clean.service <<EOF
[Unit]
Description=Quay Clair Temp File Cleanup
After=network-online.target

[Service]
Type=oneshot
ExecStart=${CLAIR_TMP_WRAPPER}
Nice=10
...
EOF
  ...
  cat >/etc/systemd/system/quay-clair-tmp-clean.timer <<'EOF'
[Unit]
Description=Hourly Quay Clair Temp File Cleanup
...
EOF
  ...
  systemctl enable --now quay-clair-tmp-clean.timer >/dev/null 2>&1 || true
else
  printf '0 3 * * * root %s >/dev/null 2>&1\n' "$GC_WRAPPER" >/etc/cron.d/quay-gc
  \chmod 644 /etc/cron.d/quay-gc
  printf '0 * * * * root %s >/dev/null 2>&1\n' "$CLAIR_TMP_WRAPPER" >/etc/cron.d/quay-clair-tmp-clean
  \chmod 644 /etc/cron.d/quay-clair-tmp-clean
fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
🧹 Clair tmp cleanup:  ENABLED (hourly, tmpfs cap ${CLAIR_TMPFS_SIZE})
